...
void cancelReservation(std::vector<Movie>& movieVec, std::vector<Reservation>& reserved) {
    ...
    for (Reservation& reservation : reserved) {
        std::cout << reservation.customerName <<
                  ", " << reservation.movieTitle <<
                  ", 2024-03-0" << reservation.date + 1 <<
                  ", " << reservation.round + 1 <<
                  ", " << reservation.number << std::endl;
    }
...
void saveReservationsData(const std::vector<Reservation>& reservations, const std::string& filename) {
    ...
    for (const Reservation& reservation : reservations) {
        if (!reservation.customerName.empty()) { // Only save reservations with non-empty customer name
...
void loadMoviesData(std::vector<Movie>& movies, const std::string& filename) {
    ...
    while (getline(file, line)) {
        ...
        while (getline(iss, token, ',')) {
            ...
            if (seatsPerDay.size() == 4) { // Assuming 4 rounds per day as per initial setup
...
int main() {
    ...
    if (movies.empty()) {
        movies = {
            ...
            Movie{"Oppenheimer", 3, {{10, 10, 10, 10}, {10, 10, 10, 10}, {10, 10, 10, 10}}},
            Movie{"Spiderman", 3, {{10, 10, 10, 10}, {10, 10, 10, 10}, {10, 10, 10, 10}}}
        };
    }
